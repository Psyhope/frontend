...
const AuthContext = createContext<{
    ...
    login: undefined as unknown as (username: string, password: string) => Promise<void>,
    logout: undefined as unknown as () => void,
    register: undefined as unknown as (username: string, password: string, confirmPassword: string, email: string) => Promise<void>,
...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
    ...
    const [user, setUser] = useState<{ username: string, email: string, id: string }>({ username: "", email: "", id: "" });
    ...
    const login = async (username: string, password: string) => {
        ...
        try {
            ...
            // console.log(data)
            const user = jwt_decode<{ username: string, email: string, sub: string }>(data.access_token);
            // console.log(user)
    ...
    const refreshToken = async () => {
        try {
            ...
            const data: TokenResponse = await res.json();
            const user: { username: string, email: string, sub: string } = jwt_decode(data.access_token);
            // console.log(user)
            setUser({ username: user.username, email: user.email, id: user.sub });
            setAccessToken(data.access_token);
        } catch (error) {
